...
// Opening title card (first ~1.8s) — the ranking's promise.
const TitleCard = ({ text }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 'rgba(4,3,10,0.72)', justifyContent: 'center', alignItems: 'center', opacity: out }}>
...
// One ranked item: looped clip + crown rank badge + label + caption. Captions
// ARE the info (no voiceover), so they're big and legible.
const ItemScene = ({ item, clip, fps }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: inOp }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.65) 0%, transparent 22%, transparent 58%, rgba(0,0,0,0.85) 100%)', pointerEvents: 'none' }} />
      {/* rank badge */}
      <div style={{ position: 'absolute', top: 120, left: 0, right: 0, display: 'flex', justifyContent: 'center', transform: `scale(${badgeScale})` }}>
        <div style={{ textAlign: 'center' }}>
          {isWinner && <div style={{ fontSize: 76, lineHeight: 1, marginBottom: -12, filter: 'drop-shadow(0 4px 14px rgba(0,0,0,0.9))' }}>👑</div>}
          <div style={{ fontFamily, fontWeight: 900, letterSpacing: -6, fontSize: isWinner ? 160 : 124, color: isWinner ? GOLD : '#fff', WebkitTextStroke: `5px ${isWinner ? '#3a2c00' : GOLD}`, paintOrder: 'stroke fill', textShadow: '0 8px 34px rgba(0,0,0,0.95)' }}>#{rank}</div>
        </div>
      </div>
      {/* label + caption */}
      <div style={{ position: 'absolute', bottom: 130, left: 40, right: 40, textAlign: 'center' }}>
        <div style={{ fontFamily, fontWeight: 900, fontSize: 76, color: '#fff', textTransform: 'uppercase', letterSpacing: -1, lineHeight: 1.02, textShadow: '0 4px 22px rgba(0,0,0,1)' }}>{item?.label || ''}</div>
        {item?.caption ? (
          <div style={{ fontFamily, fontWeight: 700, fontSize: 42, color: GOLD, marginTop: 12, textShadow: '0 3px 16px rgba(0,0,0,1)' }}>{item.caption}</div>
        ) : null}
      </div>
    </AbsoluteFill>
  );
...
// Kings of Ranks — SILENT ranking: no voiceover, music + captions + crown ranks
// over legal stock/CC clips. items ordered #5 -> #1, aligned by index with clips.
export const KingsRanks = ({
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#000' }}>
      ...
      {/* persistent crown watermark */}
      <div style={{ position: 'absolute', top: 46, left: 0, right: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, pointerEvents: 'none', zIndex: 30 }}>
