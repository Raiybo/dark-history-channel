...

{
  ...
  "narration": "60-75 words MAX — shorter Shorts get watched to the end, and completion rate is the #1 signal that makes the algorithm push a video. AUDIENCE IS PRIMARILY AMERICAN, so: use IMPERIAL units (miles, feet, pounds, Fahrenheit) not metric; compare sizes/distances/speeds to AMERICAN reference points a US viewer instantly pictures (the size of Texas, taller than the Statue of Liberty, longer than a football field, hotter than Phoenix in July, faster than a Ferrari, wider than the Grand Canyon); use casual conversational American English (contractions, 'y'all can'-adjacent phrasing is fine, no British spellings). OPEN with the hook line verbatim (Title Case OK in narration, but the wording must be IDENTICAL to hook_text). The opening sentence must read as natural spoken English — never robotic, never word-jammed. Then in the VERY NEXT breath state the surprising VISUAL payoff (what we are about to SEE) so the viewer never waits for it. Then explain WHY it is true in vivid, concrete, simple words and add ONE extra detail that makes it cooler. Short punchy sentences, high energy, like an excited friend. END on a tight line that loops back to the opening idea so a rewatch feels natural, with a 2-3 word follow nudge baked in — e.g. 'Nature hides more like this. Follow for more.' Keep the outro SHORT; long endings get skipped. Use plain spoken words and simple punctuation only (no dashes, parentheses, or symbols — they make the voice stumble). IMPORTANT: separate each idea shift with ' || ' (a literal double-pipe) marking a short breath. Use 4 to 6 of them, including one right after the opening hook line. Never put || inside a single idea, and never start or end the narration with ||.",
  "scenes": [
