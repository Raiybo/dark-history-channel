...

{
  ...
  "narration": "90-135 words, spoken as a fast countdown. STRUCTURE (each part separated by a literal ' || ' double-pipe): open with the hook line verbatim (identical wording to hook_text) as a quick promise || 'Number five,' + name the item + the ONE surprising visual thing about it, one tight sentence || 'Number four,' + item ... || 'Number three,' + item ... || 'Number two,' + item ... || 'Number one,' + the most jaw-dropping item + why it wins || a 3-4 word loop-back with a follow nudge, e.g. 'Number one always wins. Follow for more.' RULES: put ' || ' between the intro and EACH numbered item and before the outro (that is 6 double-pipes total). Never put || inside a single item, and never start or end the narration with ||. Each item MUST name its concrete visual subject so we can show it. Plain spoken words and simple punctuation only — no dashes, parentheses, or symbols (they make the voice stumble).",
  "scenes": [
