...

{
  ...
  "narration": "AIM for about 110 words total (range 95-135, never below 90, never over 140) so the video runs 40-55 seconds — long enough to clear the algorithm's watch-time bar but staying inside 60. Spoken as a fast countdown. STRUCTURE (each part separated by a literal ' || ' double-pipe): open with the hook line verbatim (identical wording to hook_text) as a quick promise || 'Number five,' + name the item + one or two short punchy sentences on the surprising visual thing about it || 'Number four,' + item ... || 'Number three,' + item ... || 'Number two,' + item ... || 'Number one,' + the most jaw-dropping item + why it wins || a 3-4 word loop-back with a follow nudge, e.g. 'Number one always wins. Follow for more.' Keep each item to one or two short sentences — enough to be vivid, tight enough to stay under 140 words total. RULES: put ' || ' between the intro and EACH numbered item and before the outro (that is 6 double-pipes total). Never put || inside a single item, and never start or end the narration with ||. Each item MUST name its concrete visual subject so we can show it. Plain spoken words and simple punctuation only — no dashes, parentheses, or symbols (they make the voice stumble).",
  "scenes": [
